...
public class CustomPointsforRows extends Base {
	WebDriver driver;
	WebDriver driver1;
	WebDriverWait wait, wait1;
	int int2, expectedtotalscore, calculate, exp=0, calc;
	String s6, s7, GameName, GameURL, twitterLink, PintrestLink, t1, p1, parent, expectedValue, actualValue, v1="10", v2="20",
			 v3="30",  v4="40",  v5="50";
	WebElement ElementNotGoingToVisible;
	Actions act;

	public static Logger Log = LogManager.getLogger(CustomPointsforRows.class.getName());
	...
	@Test
	public void Verify_CustomPointsforRows() throws InterruptedException, IOException {
	...
	public void modeaterscreen() throws InterruptedException {
		...
		Select values = new Select(driver.findElement(By.cssSelector("#custom_points_for_rows > span.selectWrapperWithLabel > select")));
		values.selectByIndex(i);
		Thread.sleep(2000);
		
		WebElement toClear = driver.findElement(By.xpath("//input[@class='ml-1 customInput']"));
		toClear.sendKeys(Keys.CONTROL + "a");
		toClear.sendKeys(Keys.DELETE);
			//	driver.findElement(By.xpath("//input[@class='ml-1 customInput']")).clear()	;	
				Thread.sleep(2000);
				
driver.findElement(By.xpath("//input[@class='ml-1 customInput']")).sendKeys("v"+k);
int m = k+10;
k=m;
		Thread.sleep(3000);
		
		//String actual = driver1.findElement(By.xpath(		"(//*[contains(@class,'gameQuestionBlock')])[" + j + "]"))
		}
			...
			if (!parent.equalsIgnoreCase(child)) {
				...
				for (int j = 1; j <= TotalTiles; j++) {
					...
					String actual = driver.findElement(By.xpath(
							"(//*[contains(@class,'gameQuestionBlock')])[" + j + "]"))
							.getText();
					System.out.println("value is >>" + actual);
					assertTrue(actual.equals("$10") || actual.equals("$20") || actual.equals("$30")
							|| actual.equals("$40") || actual.equals("$50"));
				}
				...
				for (int m = 1; m <= 5; m++) {
					 ...
					 driver.findElement(By.xpath("(//*[contains(@class,'gameQuestionBlock')])[" + m + "]"))
							.click();
						Thread.sleep(2000);
						driver1.switchTo().window(driver1.getWindowHandle());
						Thread.sleep(3000);
						...
						String scoreInsidePodium = driver
								.findElement(By.xpath("//div[@class='podium-scoreboard-score']")).getText();
						assertTrue(scoreInsidePodium.equals("$10")|| scoreInsidePodium.equals("$30") || scoreInsidePodium.equals("$60")
								|| scoreInsidePodium.equals("$100") || scoreInsidePodium.equals("$150"));
				}
				
				System.out.println("Custom Points for rows is working fine");
				Log.info("Custom Points for rows is working fine");
