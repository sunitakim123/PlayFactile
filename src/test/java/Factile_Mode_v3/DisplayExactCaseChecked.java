...
public class DisplayExactCaseChecked extends Base {
	WebDriver driver;
	WebDriver driver1;
	...
	public static Logger Log = LogManager.getLogger(DisplayExactCaseChecked.class.getName());
	...
	public void modeaterscreen() throws InterruptedException {
		...
		WebElement s1 = driver.findElement(
				By.xpath("//label[contains(text(), 'Display Exact Case')]/preceding-sibling::input[@type='checkbox']"));
	...
	public void PlayerScreen() throws InterruptedException, IOException {
		...
		for (String child : allWindows) {
			if (!parent.equalsIgnoreCase(child)) {
				...
				Thread.sleep(2000);
				WebElement store = driver1.findElement(By.xpath("//div[@class='text-content-wrapper']"));

				String actual = store.getAttribute("style");
				System.out.println("actual fetch value" + actual);

				 String expected = "color: rgb(255, 255, 255); font-size:";

				// assertTrue(actual.equals(expected));

				Assert.assertTrue(actual.contains(expected));
				Log.info("Display Exact Case>> When Checked");
				System.out.println("Display Exact Case>> When Checked");
