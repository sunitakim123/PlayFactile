...
public class PlayAsNumberedTilesInsteadOfJeopardyStyleChecked extends Base {
	WebDriver driver;
	WebDriver driver1;
	...
	public static Logger Log = LogManager.getLogger(PlayAsNumberedTilesInsteadOfJeopardyStyleChecked.class.getName());
	...
	@Test
	public void PlayAsNumberedTiles_Instead_Of_JeopardyStyle_checked_At_Global_Settings() throws InterruptedException, IOException {
	...
	public void modeaterscreen() throws InterruptedException {
		...
		Thread.sleep(2000);
		System.out.println(driver.findElement(By.xpath("//label[text()='Play As Numbered Tiles Instead Of Jeopardy-style']")).getText());

		WebElement s1 = driver.findElement(
				By.xpath("//label[contains(text(), 'Play As Numbered Tiles Instead Of Jeopardy-style')]/preceding-sibling::input[@type='checkbox']"));
		...
		Log.info("At customization page>> Checked Play As Numbered Tiles Instead Of Jeopardy-style >> ");
	}
	...
	public void PlayerScreen() throws InterruptedException, IOException {
		...
		for (String child : allWindows) {
			if (!parent.equalsIgnoreCase(child)) {
				...
				String NumberOnTileAtModerator = driver.findElement(By.xpath("(//span[@data-points='100'])[1]")).getText();
				System.out.println("AT Moderater side" + NumberOnTileAtModerator);
				String ExpectedTitle = "1";

				Assert.assertEquals(ExpectedTitle, NumberOnTileAtModerator);

				driver1.switchTo().window(driver1.getWindowHandle());
				Thread.sleep(3000);

				String NumberOnTileAtPlayer = driver1.findElement(By.xpath("(//span[@data-points='100'])[1]")).getText();
				System.out.println("At Player side:-" + NumberOnTileAtPlayer);

				Assert.assertEquals(NumberOnTileAtPlayer, NumberOnTileAtModerator);
				Log.info("Play as Numbered Tiles Instead OF Jeopardy Style");
				System.out.println("Play as Numbered Tiles Instead OF Jeopardy Style"); 
