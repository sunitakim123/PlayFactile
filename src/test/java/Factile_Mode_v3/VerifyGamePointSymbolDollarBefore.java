...
public class VerifyGamePointSymbolDollarBefore extends Base {
	WebDriver driver;
	WebDriver driver1;
	...
	public static Logger Log = LogManager.getLogger(VerifyGamePointSymbolDollarBefore.class.getName());
	...
	@Test
	public void Verify_Game_Point_Symbol_$__Before() throws InterruptedException, IOException {
	...
	public void modeaterscreen() throws InterruptedException {
		...
		driver.findElement(By.xpath("//button[@class='swal2-confirm swal2-styled']")).click();
		Thread.sleep(2000);
		wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//input[@id='scoreboard_symbol']")));
		driver.findElement(By.xpath("//input[@id='scoreboard_symbol']")).clear();
		Thread.sleep(2000);
		
	
		driver.findElement(By.xpath("//input[@id='scoreboard_symbol']")).sendKeys(CustomSymbol);
		Thread.sleep(3000);
		Select values = new Select(driver.findElement(By.xpath("//select[@class='selectBox custom-select']")));
		...
		for (String child : allWindows) {
			if (!parent.equalsIgnoreCase(child)) {
				...
				String scoreInsidePodiumAtEnd = driver.findElement(By.xpath("//div[@class='podium-scoreboard-score']"))
						.getText();
				System.out.println("Score inside podium:-"+ scoreInsidePodiumAtEnd);
				
				String actualcustomsymbol= scoreInsidePodiumAtEnd.substring(0,1);
				 System.out.println("Substring starting from index 0 to 0: "+actualcustomsymbol); //Sachin  
				Assert.assertEquals(CustomSymbol, actualcustomsymbol);
				Log.info("Game_Point_Symbol_$_Before is working fine");
				System.out.println("Game_Point_Symbol_$_Before is working fine");
