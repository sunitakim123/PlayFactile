...
public class DGBP_HSB_TestCase_7 extends Base {
	WebDriver driver;
	WebDriver driver1;
	WebDriverWait wait, wait1;
	int int2;
	String s6, s7, GameName, twitterLink, PintrestLink, t1, p1, parent, expectedValue;
	WebElement ElementNotGoingToVisible;
	Actions act;

	public static Logger Log = LogManager.getLogger(DGBP_HSB_TestCase_7.class.getName());
	...
	@Test
	public void TC_7_Verify_Exact_Case_When_checked_At_Global_Settings() throws InterruptedException, IOException {
	...
	public void PlayerScreen() throws InterruptedException, IOException {
		...
		for (String child : allWindows) {
			if (!parent.equalsIgnoreCase(child)) {
				...
				Thread.sleep(2000);
				// expectedValue = "text-transform: uppercase;";
				try {
					ElementNotGoingToVisible = driver1
							.findElement(By.xpath("//div[@style='text-transform: uppercase;']"));
				} catch (NoSuchElementException e) {
					System.out.println("Element Not visible " + e.toString());
				}
