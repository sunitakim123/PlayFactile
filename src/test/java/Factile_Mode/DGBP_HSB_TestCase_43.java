...
public class DGBP_HSB_TestCase_43 extends Base {
	WebDriver driver;
	WebDriver driver1;
	...
	public static Logger Log = LogManager.getLogger(DGBP_HSB_TestCase_43.class.getName());
	...
	@Test
	public void TC_43_Verify_wager_submission_Half_amount_by_player() throws InterruptedException, IOException {
	...
	public void PlayerScreen() throws InterruptedException, IOException {
		...
		for (String child : allWindows) {
			if (!parent.equalsIgnoreCase(child)) {
				...
				String amount = driver1.findElement(By.xpath("//span[@class='moneyHolder']")).getText();
				String moneyvalue = amount.substring(1);
				System.out.println("moneyholder:=" + moneyvalue);
				int money = Integer.parseInt(moneyvalue);
				int calculate = money / 2;
				int expectedvalue=money+calculate;
				//System.out.println("wager value:=" + calculate);
				String sendvalue = Integer.toString(calculate);
				System.out.println("wager submitted by player:=" + sendvalue);
				wait1.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//input[@class='wagerInput']")));
				driver1.findElement(By.xpath("//input[@class='wagerInput']")).sendKeys(sendvalue);
