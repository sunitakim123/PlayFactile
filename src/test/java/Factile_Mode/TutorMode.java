...
public class TutorMode extends Base {
	WebDriver driver;
	WebDriver driver1;
	...
	public static Logger Log = LogManager.getLogger(TutorMode.class.getName());
	...
	@Test
	public void Tutor_Mode_checked_At_Global_Settings() throws InterruptedException, IOException {
	...
	public void modeaterscreen() throws InterruptedException {
		...
		driver.findElement(By.xpath("//button[@class='swal2-confirm swal2-styled']")).click();
		Thread.sleep(2000);
		System.out.println(driver.findElement(By.xpath("//label[text()='Tutor Mode']")).getText());

		WebElement s1 = driver.findElement(
				By.xpath("//label[contains(text(), 'Tutor Mode')]/preceding-sibling::input[@type='checkbox']"));
		...
		Log.info("At customization page>> Tutor Mode is checked >> ");
	}
	...
	public void PlayerScreen() throws InterruptedException, IOException {
		...
		for (String child : allWindows) {
			if (!parent.equalsIgnoreCase(child)) {
				...
				for (int j = 0; j <= 1; j++) {
					for (int k = 1; k <= 6; k++) {
						System.out.println("inside 2nd loop value of l:-" + l);

						driver.findElement(By.xpath("(//span[@data-points='" + l + "'])[" + k + "]")).click();
						Thread.sleep(2000);
						Thread.sleep(2000);
					...
					driver.findElement(By.xpath("//span[text()='Reset Game']")).click();
					Thread.sleep(2000);
					driver.findElement(By.xpath("//button[@class='swal2-confirm swal2-styled']")).click();
					Thread.sleep(2000);
					String scoreInsidePodiumafterreset = driver
							.findElement(By.xpath("//div[@class='podium-scoreboard-score']")).getText();
					System.out.println("Actual reset value inside podium:-" + scoreInsidePodiumafterreset);
					Assert.assertEquals(expectedReset, scoreInsidePodiumafterreset);

				}
				System.out.println("Tutor Mode is Working fine");
				Log.info("Tutor Mode is Working fine");
			}
		}
	}
}
