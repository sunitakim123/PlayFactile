...
public class DGBP_HSB_TestCase_40 extends Base {
	WebDriver driver;
	WebDriver driver1;
	...
	@Test
	public void TC_40_MultiTeamMemeber_PlayerSelctedTeamAssignmnet() throws InterruptedException, IOException {
...
/*
	...
	*/
	...
	public void modeaterscreen() throws InterruptedException {
		...
		if (driver.findElement(By.xpath("//input[@name='multiTeamManage']")).isSelected()) {
			System.out.println("Player Selected Team Assignment >> already selected");
		} else {
			act1 = new Actions(driver);
			act1.moveToElement(driver.findElement(By.xpath("//input[@name='multiTeamManage']"))).click().perform();
		}
	...
	public void PlayerScreen() throws InterruptedException, IOException {
		...
		for (String child : allWindows) {
			...
			System.out.println("user first mascot name="+mascotnamefisrt);
				...
				Thread.sleep(2000);
				driver3.findElement(By.xpath("(//div[@class='joinGameAsPoints'])[2]")).click();
				Thread.sleep(2000);
			int sizeofmascots=	driver3.findElements(By.xpath("//div[@class='chooseChar']")).size();
			System.out.println("on team member screen total number of mascots visible="+sizeofmascots);
			String expectedmascotNameFirst=	driver3.findElement(By.xpath("(//div[@class='characterBlock position-relative'])[1]")).getAttribute("data-name");
			String expectedmascotNamesecond=	driver3.findElement(By.xpath("(//div[@class='characterBlock position-relative'])[2]")).getAttribute("data-name");
				Assert.assertEquals(expectedmascotNameFirst, mascotnamefisrt);
				Assert.assertEquals(expectedmascotNamesecond, mascotnamesecond);
				...
				//	Thread.sleep(2000);
					...
					wait2.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//p[@class='answerSubmitMsg']")));
						String MsgOnCaptain2Screen = driver2.findElement(By.xpath("//p[@class='answerSubmitMsg']")).getText();
					System.out.println("msg on captain screen="+MsgOnCaptain2Screen);
					driver3.switchTo().window(driver3.getWindowHandle());
					
					wait3.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//p[@class='answerSubmitMsg']")));
					String MsgOnTeamMemeberOfCaptain2Screen = driver3.findElement(By.xpath("//p[@class='answerSubmitMsg']")).getText();
					System.out.println("msg on Team memeber screen="+MsgOnTeamMemeberOfCaptain2Screen);
					Assert.assertEquals(MsgOnTeamMemeberOfCaptain2Screen, MsgOnCaptain2Screen);
					...
					else
					{
						...
						wait2.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//p[@class='answerSubmitMsg']")));
						String MsgOnCaptain2Screen = driver2.findElement(By.xpath("//p[@class='answerSubmitMsg']")).getText();
						System.out.println("msg on Player screen="+MsgOnCaptain2Screen);

						Assert.assertEquals(MsgOnCaptain2Screen, MsgOnTeamMemeberOfCaptain2Screen);
